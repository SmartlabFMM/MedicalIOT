...
old = "med_iot_command_center/static/src/js/logout_redirect_home.js"

for bundle in ["web.assets_backend", "web.assets_web"]:
    items = assets.setdefault(bundle, [])
    items = [x for x in items if x not in [old, safe]]
    items.append(safe)
    assets[bundle] = items
...
print("logout redirect JS forced in both bundles")