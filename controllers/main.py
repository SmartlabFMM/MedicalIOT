...
class MedIoTAuthController(http.Controller):
    ...
    def mediot_post_login(self, **kwargs):
        ...
        if user.has_group('MedicalIOT.group_med_admin'):
            action = request.env.ref('MedicalIOT.action_med_admin_dashboard').sudo()
            menu = request.env.ref('MedicalIOT.menu_med_admin_dashboard').sudo()
            return request.redirect(f'/web#action={action.id}&menu_id={menu.id}')

        if user.has_group('MedicalIOT.group_med_senior_doctor'):
            action = request.env.ref('MedicalIOT.action_med_dashboard').sudo()
            menu = request.env.ref('MedicalIOT.menu_med_dashboard').sudo()
            return request.redirect(f'/web#action={action.id}&menu_id={menu.id}')
